...
# Here, we construct the model (What we originally used before we started using the saved weights)
# model = Sequential()
...
model = load_model('lstm_layered.h5')
adam = optimizers.Adam(lr=0.0005, decay = 0.000025)
...
print (devLoss)
np.savetxt("dev_preds_lstm_layered.csv", devGuesses, delimiter=",")
np.savetxt("dev_acts_lstm_layered.csv", devY, delimiter=",")
devResults = open("dev_results_lstm_layered.txt", 'w')
...
print (testLoss)
np.savetxt("test_preds_lstm_layered.csv", testGuesses, delimiter=",")
np.savetxt("test_acts_lstm_layered.csv", testY, delimiter=",")
testResults = open("test_results_lstm_layered.txt", 'w')
