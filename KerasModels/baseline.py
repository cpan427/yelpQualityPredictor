...
model = Sequential()
embed = Embedding((vocab_size+1), output_dim, weights=[embedding_matrix], input_length = 50, embeddings_initializer='random_uniform')
model.add(embed)
#print (embed.output_shape)
model.add(Flatten())
model.add(Dense(1, activation='relu'))

adam = optimizers.Adam(lr=0.002, amsgrad=True)
model.compile(optimizer=adam,
              loss='mse')
print(model.summary())
model.fit(x=padded_docs,y=trainY, batch_size = 256, epochs=1)
...
print (devLoss)
np.savetxt("dev_preds_baseline.csv", devGuesses, delimiter=",")
np.savetxt("dev_acts_baseline.csv", devY, delimiter=",")
devResults = open("dev_results_baseline.txt", 'w')
...
print (testLoss)
np.savetxt("test_preds_baseline.csv", testGuesses, delimiter=",")
np.savetxt("test_acts_baseline.csv", testY, delimiter=",")
testResults = open("test_results_baseline.txt", 'w')
