...
def load_dataset_from_text(path_txt, vocab=None, split=True):
    ...
    if vocab is not None:
        ...
        dataset = dataset.map(lambda string: (tf.string_to_number(string, tf.int32), tf.constant(0, dtype=tf.int32)))
...
def input_fn(mode, sentences, labels, params):
    ...
    padded_shapes = ((tf.TensorShape([50]),  # sentence of unknown size
                      tf.TensorShape([])),     # size(words)
                     (tf.TensorShape([]),      # labels of unknown size
